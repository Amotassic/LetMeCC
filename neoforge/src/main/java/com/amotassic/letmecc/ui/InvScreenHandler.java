...
import static com.amotassic.letmecc.ModTools.isCuriosLoaded;
...
public class InvScreenHandler extends AbstractContainerMenu {
    ...
    public InvScreenHandler(int syncId, Inventory inv, FriendlyByteBuf buf) {
        ...
        for (Object[] data : slotData) {
            ...
            addSlot(new Slot(inventory, slotId, 8 + xOffset * 18, 18 * armorRow).setBackground(BLOCK_ATLAS, texture));
        }
        // BodyArmor槽位
        ...
        var pairs = trinketsWithSlots(target);
        outer: for (i = 0; i < 99; i++) { //饰品栏
            for (j = 0; j < 9; j++) {
                ...
                AtomicReference<ResourceLocation> texture = new AtomicReference<>(null);
                if (inv.player.level().isClientSide) CuriosApi.getSlot(pairs.get(j + i * 9).getA().getIdentifier(), true).ifPresent(iSlotType -> texture.set(iSlotType.getIcon()));
                addSlot(new Slot(inventory, index, x, y).setBackground(BLOCK_ATLAS, texture.get()));
            }
        }
    }
    ...
    @Override public boolean stillValid(Player player) {
        if (!slotsEnabled.equals(new EntityInventory(target, editable).slotIndexes))
    ...
    public static class EntityInventory implements Container {
        ...
        public ItemStack getItem(int slot) {
            if (slotIndexes.contains(slot)) {
                ...
                if (0 < slot && slot <= 35) {
                    ...
                    if (owner instanceof AbstractHorse horse && slot <= horse.getInventory().getContainerSize()) {
                        return horse.getInventory().getItem(slot - 1);
                    }
                }
                ...
                if (slot >= TRINKET_INDEX && isCuriosLoaded()) {
                    var pair = trinketsWithSlots(owner).get(slot - TRINKET_INDEX);
                    return pair.getA().getStacks().getStackInSlot(pair.getB());
                }
            }
            return ItemStack.EMPTY;
        }
        ...
        public void setItem(int slot, ItemStack stack) {
            ...
            if (0 < slot && slot <= 35) {
                ...
                if (owner instanceof AbstractHorse horse && slot <= horse.getInventory().getContainerSize()) {
                    horse.getInventory().setItem(slot - 1, stack);
                }
            }
            ...
            if (slot >= TRINKET_INDEX && isCuriosLoaded()) {
                var pair = trinketsWithSlots(owner).get(slot - TRINKET_INDEX);
                pair.getA().getStacks().setStackInSlot(pair.getB(), stack);
            }
        }
